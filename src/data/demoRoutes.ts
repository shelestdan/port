...
const seaAdditionalExpenses =
  "Базовый тариф FESCO: ЖД-перевозка, экспедирование в порту, погрузка на судно во Владивостоке, морская перевозка и контейнер. CY включает терминальную обработку на станции отправления. Полные условия ниже.";

const railAdditionalExpenses =
  "Базовый тариф прямого ЖД-направления. Дополнительные сборы и терминальные операции уточняются по заявке.";

export const fescoTerms = [
  "FOR станция отправления – FO порт назначения включает ЖД-перевозку, экспедирование в порту, погрузку на судно во Владивостоке, морскую перевозку и предоставление контейнера FESCO.",
  "FOR станция отправления – FOR порт Владивосток включает ЖД-перевозку и предоставление контейнера FESCO. Тарифы CY дополнительно включают терминальную обработку контейнера на станции отправления.",
  "Хранение во Владивостоке: 18 суток свободно, далее 1300/2600 руб. за 20/40-фут. контейнер в день. При снятии с перевозки или изменении направления хранение с 1-х суток: 4400/5400 руб. за 20/40-фут. контейнер в день; не действует для контейнеров, сменивших статус с груженого на порожний.",
  "Отмена экспортной букировки после CUT OFF: 1500 руб. за контейнер. Корректировка коносамента: первая до выхода из порта бесплатно, далее 7400 руб.; смена порта назначения — с первой корректировки.",
  ...
  "Пломбировка: 1100 руб.; маркировка: 2400 руб.; знаки опасности: 850 руб. за комплект. Дополнительное перемещение для работ: 6400/9600 руб.; дополнительная сортировка при отсутствии или ошибке информирования: 3500 руб.; сортировка внутри терминала: 2200 руб.",
...
function sea(
  ...
  return {
    ...
    additionalExpenses: seaAdditionalExpenses,
...
function rail(
  ...
  return {
    ...
    additionalExpenses: railAdditionalExpenses,
